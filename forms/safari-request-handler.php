...
// Send to company
$companyBody = '<h2>New Safari Request</h2><table style="border-collapse:collapse;width:100%">';
...
foreach ($fields as $k => $v) {
    if ($v) $companyBody .= '<tr><td style="padding:8px 12px;border:1px solid #ddd;font-weight:600;background:#f9f9f9;width:140px">'.$k.'</td><td style="padding:8px 12px;border:1px solid #ddd">'.$v.'</td></tr>';
}
$companyBody .= '</table>';
$sent = sendMail(COMPANY_EMAIL, 'Safari Request: ' . $journey . ' — ' . $name, $companyBody, $email);

// Auto-reply to visitor
$details = '<div style="background:#f9f9f9;border-radius:8px;padding:1rem;margin:1rem 0"><p style="margin:0;font-size:0.875rem;color:#555"><strong>Journey:</strong> '.$journey.'<br><strong>Destination:</strong> '.$dest.'<br><strong>Duration:</strong> '.$duration.'</p></div>';
